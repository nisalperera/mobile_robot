...
def compute_multiplier(
    ...
    # Position error (only meaningful for --mode arc; harmless to compute
    # for --mode spin too, since it should be ~0 there).
    ...
    dist_truth = math.hypot(dx_truth, dy_truth)
...
def print_result(result: dict, mode: str):
    ...
    if mode == "arc":
        print(f"\n  Path length (truth): {result['dist_truth_m']:.4f} m")
        print(f"  Path length (odom):  {result['dist_odom_m']:.4f} m")
